...
% Extended by Michael Hatcher (m.c.hatcher@soton.ac.uk) to deal with N states in simulations
...
nz = 3; % No. of states of technology 
...
%--------------------------------
%Three states, nz = 3, example
%Z = [0.975 1 1.025];
%PI = [0.9 0.06  0.04; 0.8 0.15 0.05; 0.1 0.04 0.86];

Z = linspace(0.95,1.05,nz); %states within 5% of mean of 1
M = rand(nz); %matrix with random values in (0,1) 
for i=1:nz 
    for j=1:nz 
  PI(i,j) = M(i,j)/sum(M(i,1:nz)); %generate valid transition matrix 
    end
end
...
% There are N states for z, z1, z2,...,zN. We used the uniform dist. on (0,1) to determine the realized state. 
