...
% Code originally written by Panayiotis Pourpourides, pourpouridesp@cf.ac.ac.uk, and amended by Michael Hatcher
...
nz = 2; % No. of states of technology % if you change this number you also need to change Z and PI
...
%Three states, nz = 3
%Z = [0.975 1 1.025];
%PI = [0.9 0.06  0.04; 0.8 0.15 0.05; 0.1 0.04 0.86];
...
% There are two states for z, z1 and z2. Suppose that at i = 0, z1 was realized.
%At i = 1, draw a random number r2[0; 1]. If r is less than PI_11 this means that
%the economy remains in state z1. If r is greater than or equal to PI_11, then
%technology switches to z2. Given, the realized state of z and the state of k we
%pick the optimal k0 whose location in K is given by iDecK(iz,ik). At i = 2,
%current k is the previous period k0 . The same procedure is repeated T times.

% Entry simiZ(i-1) tells us what state we are currently in, just prior to drawing the new state. 
%If we are currently in state 1 and we draw r<PI we will stay in state 1 because r<PI(1,1). 
%If we are currently in state 2 and we draw r<PI we will move to state 1 because r<PI(2,1).
%Otherwise we will move to state 2, since r>PI implies that the second column of the PI matrix 
%applies - i.e. we will either move from state 1 to state 2 or, if we are already in state 2, 
%we will stay there in the next period.  
